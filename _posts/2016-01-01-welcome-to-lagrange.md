---
layout: post
title: "About"
categories: journal
tags: [documentation,sample]
image: sunset.png
---

I’m a Geography and Computer Science (Geomatics) student at the University of Victoria, with interests in surveying, geospatial analysis, mapping technologies, and art. **You can view some of the projects I’ve worked on below.**

## Past Experience

### **Coordinator, Community Mapping (GIS)**

**Canadian Red Cross** — *Victoria, BC*
**January 2025 – August 2025**

* Designed and delivered GIS training materials for diverse audiences using **Figma**.
* Supported GIS response efforts during the **2025 Alberta and Manitoba wildfires**, producing detailed maps and community profiles in **ArcGIS Pro** to support situational awareness for staff and volunteers.
* Developed interactive maps and dashboards using **ArcGIS Pro** and **ArcGIS StoryMaps**, integrating the **Social Vulnerability Index (SVI)** to visualize at-risk communities for organizational presentations.
* Redeveloped mapper databases using **Excel** and database design principles, improving data organization, accuracy, and accessibility.

---

### **Historic Computing Assistant**

**University of Victoria Libraries** — *Victoria, BC*
**October 2024 – January 2025**

* Documented historic computers for archival preservation.
* Created an interactive tutorial for the **Microstar graphics program** using **HTML, JavaScript, and CSS**.

---

### **GIS Research Assistant**

**(Surreal) Simulating Resilient Landscapes Lab, University of Victoria** — *Victoria, BC*
**November 2023 – January 2024**

* Collaborated with the Surreal Lab to identify and flag errors in **LiDAR datasets** for a **Capital Regional District** project.
* Conducted spatial data quality control and analysis using **ArcGIS Pro**.

---

### **Assistant GIS Analyst (Co-op)**

**Fisheries and Oceans Canada** — *Sidney, BC*
**January 2023 – August 2023**

* Worked with the **Salish Sea Initiative Interactive Map** team to develop an interactive mapping tool supporting Indigenous Nations’ access to data.
* Used **ArcGIS, QGIS, Figma, and R** to analyze, prepare, and publish spatial datasets.
* Wrote clear, accessible information panels explaining mapped data and methodologies.

---

### **Analyst (Co-op)**

**British Columbia Environmental Assessment Office** — *Victoria, BC*
**September 2021 – January 2022**

* Supported the Oil and Gas team in correspondence with Indigenous Nations and industry partners.
* Maintained and updated the Environmental Assessment Office’s **EPIC** website.
* Developed an **Excel-based tracking database** for Compliance & Enforcement environmental assessment projects.
* Served on the **Co-op Advisory Board**, organizing events and contributing to improvements in the co-op program.

---




